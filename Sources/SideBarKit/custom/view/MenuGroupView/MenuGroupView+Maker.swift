...
extension MenuGroupView {
   ...
   @ViewBuilder func rowItem(rowItem: RowKind, uuid: UUID, index: CombinedIndex) -> some View {
      ...
      case let labelType as LabelType:
         LabelRowView(labelType: labelType, selected: self.$selected, index: index, selection: self.$selection)
      case let filterType as FilteringType:
         FilterRowView(filterType: filterType, selected: self.$selected, uuid: uuid, index: index, selection: self.$selection)
      case let miscType as MiscType:
         MiscRowView(miscType: miscType, selected: self.$selected, index: index, selection: self.$selection)
